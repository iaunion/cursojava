...
public class PersonaJdbcRepository implements IPersonaRepository {
	...
	public void add(Persona persona) throws Exception {
		...
		try (Connection connection = DriverManager.getConnection("jdbc:sqlite:persona.db")){
			...
			sql.executeUpdate(String.format("INSERT INTO Persona (Nombre,Apellido) VALUES ('%s','%s')", persona.getNombre(), persona.getApellido()));
		}
	}
